...
\newtheorem{GS}[thm]{Teorema di Gale-Stewart}
...
\begin{GS}\label{sec:orgf56b48c}
Sia \(T\) un {albero} {potato} non vuoto su \(A\). Se \(C \subseteq [T]\) è {aperto} o {chiuso} in \([T]\) e {Sottospazio topologico}}, allora {il gioco} \(G(T,C)\) è {determinato}.
\end{GS}
...
%% Sia \(X\) uno \missingreference{spazio topologico}
%% \subsection{Definizione}
%% \label{sec:org6117d9d}

%% \begin{itemize}
%% \item Un \missingreference{sottoinsieme} \(A \subseteq X\) è \uline{magro} se può essere scritto come \missingreference{unione} \missingreference{numerabile} di insiemi \missingreference{mai densi}.
%% \item Un sottoinsieme \(A \subseteq X\) è \uline{comagro} se il suo \missingreference{complementare} è magro.

%% Equivalentemente, \(A\) è \uline{comagro} se e solo se contiene l'\missingreference{intersezione} di una quantità numerabile di aperti \missingreference{densi}.
%% \end{itemize}
%% \subsection{Definizione}
...
%% \item Un sottoinsieme \(A \subseteq X\) è \uline{comagro in \(U\)} se \(U\setminus A\) è magro, ovvero se esiste una sequenza di aperti \missingreference{densi} in \(U\) la cui intersezione è contenuta in \(A\).
%% \end{itemize}
...
%% Uno \missingreference{spazio topologico} \(X\) si dice \uline{Baire} se soddisfa le seguenti condizioni equivalenti:
%% \begin{enumerate}
%% \item ogni \missingreference{sottoinsieme} di \(X\) \missingreference{aperto} non \missingreference{vuoto} è \missingreference{non-magro};
%% \item ogni \missingreference{sottoinsieme} \missingreference{comagro} di \(X\) è \missingreference{denso};
%% \item l'\missingreference{intersezione} di una quantità \missingreference{numerabile} di sottoinsiemi densi e aperti di \(X\) è denso.
%% \end{enumerate}

%% In particolare, se \(X\) è non vuoto e Baire, allora l'intersezione di ogni coppia di \missingreference{\(\bm{G}_{\delta}\)} densi di \(X\) è densa, e pertanto non vuota.
%% \subsubsection{Sottospazi aperti di uno spazio di Baire è uno spazio di Baire}
%% \label{sec:orgce12667}
%% Se \(X\) è uno spazio topologico di Baire e \(A \subseteq X\) è aperto, allora \(A\) è uno \missingreference{spazio topologico} di Baire.
...
%% Dati due \missingreference{sottoinsiemi} \(A, B \subseteq X\) si dirà che
%% \begin{equation*}
%% A \mathrel{=^{*}} B
%% \end{equation*}
%% se la \missingreference{differenza simmetrica} \(A\mathrel{\triangle} B \coloneqq (A\setminus B)\cup (B\setminus A)\) è un \missingreference{insieme magro}. \footnote{Vedi ``\missingreference{Sottrazione di classi MK}'' e ``\missingreference{Unione di classi MK}''
%% :ID:       648c8718-f653-4932-9c00-900d90560720
%% :ROAM\textsubscript{ALIASES}: ``Insieme coanalitico''}
%% \subsection{Definizione}
%% \label{sec:org95bdf49}

%% Un \missingreference{sottoinsieme} \(A \subseteq X\) ha la \uline{proprietà di Baire} (\emph{Baire Property} o BP) se esiste un \uline{\missingreference{aperto}} \(U \subseteq X\) tale che
%% \begin{equation*}
%% A\mathrel{=^{*}}B.
%% \end{equation*}
...
\item Se II ha una strategia vincente per \(G^{**}_{\text{u}}(F)\), allora ha una strategia vincente {in \(G^{**}(A)\)}. Per il {Teorema \ref{sec:org5e232d7}}, \(A\) è comagro.\qedhere
\end{enumerate}
\end{proof}
